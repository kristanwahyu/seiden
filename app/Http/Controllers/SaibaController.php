...
class SaibaController extends Controller
{
	...
	{ 
		...
		->join('tbl_dipa_pmb_check_sink_saiba', 'tbl_dipa_pmb_check_sink_saiba.dipa_pembayaran_id','=','tbl_dipa_pembayaran.dipa_pembayaran_id')
		->where('tbl_dipa_pmb_check_spp.dipa_sinkronisasi_saiba','1')
		->where('tbl_dipa_pmb_check_sink_saiba.dipa_status_sinki', '0')
    ...
    // return compact('data');
    return view('pages.operator_saiba.sinkronisasi', compact('data'));
	}
	...
	public function store(Request $request){
		...
		$saiba = DipaPembayaranSyncSaiba::where('dipa_pembayaran_id', $request->id)
			->update([
            'dipa_tanggal_sinkronisasi'   => date("Y:m:d h:m:s"),
            'dipa_status_sinki'   => '1'
        ]);
	}
}
